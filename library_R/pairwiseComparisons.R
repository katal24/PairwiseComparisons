...
#' @title Matrix A for the HRE method
...
#' @description Counts the closest consistent triad
#' @param triad - vector of three values
#' @return the closest consistent triad
#' @export
...
AIJadd <- function(...){
  matrixes <- list(...)
  matrixes <- Reduce('+', matrixes)/length(matrixes)
  matrixes
}
...
AIJgeom <- function(...){
  matrixes <- list(...)
  matrixes <- Reduce('*', matrixes)^(1/length(matrixes))    # Reduce - wywoluje funkcje po kolei na elementach listy
  matrixes
}
...
#' When the M matrix is consistent every e_ij equals 1.
#' @param M - matrix
...
#' @export
localDiscrepancyMatrix <- function(matrix, resultList){
  errorMatrix <- errorMatrix(matrix,resultList)
...
#' @export
globalDiscrepancy <- function(matrix, resultList){
  max(localDiscrepancyMatrix(matrix, resultList))
}
